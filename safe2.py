...
if st.button("✅ 生成个体化给药方案"):
    ...
    try:
                ...
                try:
                    ...
                    if pred_ml < target_low:
                        ...
                        if not found:
                            ...
                            st.success(f"机器学习预测谷浓度偏高，方案已自动降级至 {剂量:.2f}g q{间隔}h 输注{输注_time}h，预测谷浓度约 {best_safe[6]:.2f} mg/L，%T>MIC {best_ft:.1f}%")
        ...
        with colB:
            ...
            idx_stast.successst.successst.successrt = np.searchsorted(t_full, start_cycle)
            idx_end = np.searchsorted(t_full, end_cycle)
            t_cycle = t_full[idx_start:idx_end] - start_cycle
            conc_cycle = conc_full[idx_start:idx_end]
            ax.plot(t_cycle, conc_cycle, 'b-', lw=2, label='Predicted Conc.')
