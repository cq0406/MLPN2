...
def add_engineered_features(df):
    ...
    base_cl_60_pred = np.select(
        [egfr_temp >= 130, egfr_temp >= 90, egfr_temp >= 60, egfr_temp >= 45, egfr_temp >= 30, egfr_temp >= 15],
        [10.0, 8.0, 7.0, 6.5, 5.5, 4.5],
        default=4.0
        )
    weight_temp = pd.to_numeric(df["体重"], errors='coerce').fillna(60)
    weight_based_cl_pred = base_cl_60_pred + 0.35 * ((weight_temp - 60.0) / 5.0)
    weight_based_cl_pred = weight_based_cl_pred.clip(lower=4.0)
    df["CL_pred"] = weight_based_cl_pred
    # Vd 也可分层，但此处保持简单，不加入
    if "感染部位" in df.columns:
    ...
    if uploaded_files:
        ...
        if not all_dfs:
            ...
            try:
                ...
                st.dataframe(df_import.head(10), width='content')
        ...
        if view_mode == "Group by Patient ID":
            if "编号" not in df_display.columns:
                ...
                for patient_id, group_df in grouped_by_id:
                    ...
                    st.dataframe(group_df_show, width='content')
                    st.divider()
        else:
        ...
        if analysis_mode == "📊 Routine Indicator Analysis":
                ...
                if target_col is None and target_display != "Explanation":
                    ...
                    if target_display in categorical_fields:
                        ...
                        if len(series) == 0:
                            ...
                            st.pyplot(fig)
                            st.dataframe(cnt.rename("Count"), width='content')
                        ...
                        if len(s) == 0:
                            ...
                            st.dataframe(s.describe().round(2), width='content')
            ...
            if df_raw.empty:
                ...
                if not num_cols:
                        ...
                        try:
                            ...
                            if existing_cols:
                                ...
                                if len(X) > 5:
                                    ...
                                    st.dataframe(imp_df, width='content')
                    ...
                    st.dataframe(stats, width='content')
    st.stop()
...
if st.session_state.show_training:
    ...
    if not df.empty:
        with st.expander("View Existing Training Data"):
            ...
            st.dataframe(df_show, width='content')
        ...
        if len(X) < 10:
            ...
            if "outcome_model" in st.session_state:
                with st.expander("View Efficacy Model Feature Importance"):
                    ...
                    st.dataframe(imp_df_cls, width='content')
                    fig, ax = plt.subplots(figsize=(10, 6))
...
if st.button("📊 计算自定义方案达标概率"):
    ...
    try:
        ...
        # 感染严重度（独立计算）
        重症_custom = is_severe(体温, WBC) if 体温 and WBC else False
        ...
        severe_factor_custom = 1.3 if 重症_custom else 1.0

        cl_custom = weight_based_cl * age_factor * severe_factor_custom

        # 稳态模拟
        t_full, conc_full = simulate_conc_full(
            dose_custom, interval_custom, cl_custom, vd_custom, wt, inf_time
        )
        ...
        ax.set_title('Meropenem Custom Regimen Steady-State (5th Dose)')
        ...
        if max_ft >= 90:
            st.success("✅ 该方案预计达标情况优秀（%T>MIC ≥ 90%）")
        elif max_ft >= 40:
            st.info("ℹ️ 该方案对敏感菌达标，对高MIC菌株可能不足")
        else:
            st.warning("⚠️ 该方案达标概率较低，建议调整剂量或延长输注时间")
...
if st.button("✅ 生成个体化给药方案"):
    ...
    try:
        ...
        if obs_c is None:
            ...
            if ml_model is not None and ml_features is not None:
                try:
                    ...
                    st.info(f"🤖 ML predicted trough: {pred_ml:.2f} mg/L")
                    ...
                    if pred_ml < target_low:
                        ...
                        if not found:
                            st.warning("Max intensity reached but still not adequate.")
                        else:
                            st.success("ML predicted low, regimen intensified to achieve ≥90% T>MIC.")

                    elif pred_ml > target_high:
                        ml_adjusted = True
                        safety_limit = 30.0
                        ...
                        if best_safe:
                            ...
                            st.success(f"ML predicted high, regimen de-escalated to {剂量:.2f}g q{间隔}h inf{输注_time}h, trough ~{best_safe[6]:.2f} mg/L, T>MIC {best_ft:.1f}%")
                        else:
                            st.warning("Cannot find safe regimen, keep original.")
                    else:
                        ml_adjusted = True
                        ...
                        best_t_full, best_conc_full = t_full, conc_full
                        st.info("ML predicted trough within ideal range, matched curve to prediction.")
                except Exception as e:
                    st.warning(f"ML prediction failed ({e}), using PPK-only regimen.")
            else:
                st.warning("⚠️ PK ML model not trained. Using PPK-only regimen.")
        ...
        if egfr_val is not None:
            if egfr_val >= 130:
                explanation += f"🔵 肾功能亢进（eGFR {egfr_val:.0f} mL/min），剂量增加至 2g q8h。\n"
            elif 90 <= egfr_val < 130:
                explanation += f"🟢 肾功能正常（eGFR {egfr_val:.0f} mL/min），维持原方案。\n"
            elif 60 <= egfr_val < 90:
                explanation += f"🟡 肾功能轻度下降（eGFR {egfr_val:.0f}），剂量调整为 1g q12h。\n"
            elif 45 <= egfr_val < 60:
                explanation += f"🟠 肾功能轻至中度下降（eGFR {egfr_val:.0f}），剂量调整为 1g q12h。\n"
            elif 30 <= egfr_val < 45:
                explanation += f"🟠 肾功能中至重度下降（eGFR {egfr_val:.0f}），剂量调整为 0.5g q8h。\n"
            elif 15 <= egfr_val < 30:
                explanation += f"🔴 肾功能重度下降（eGFR {egfr_val:.0f}），剂量调整为 0.5g q12h。\n"
            else:
                explanation += f"🔴 肾衰竭（eGFR {egfr_val:.0f}），剂量调整为 0.5g q12h。\n"
        else:
            explanation += "⚠️ 未能获得 eGFR，按肾功能正常处理。\n"
        ...
        final_trough = trough_conc if 'trough_conc' in dir() else pred_trough
        res = [
            str(c3h) if c3h and str(c3h).strip() not in ["无", ""] else "N/A",
            f"{final_trough:.2f} mg/L (simulated)" if obs_c is None else f"{final_trough:.2f} mg/L",
            f"{best_ft:.1f}%",
