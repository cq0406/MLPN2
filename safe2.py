    ...
    if not st.session_state.eval_started:
        ...
        if df_pat.empty:
            ...
            if not dates:
                ...
                if st.button("✅ 开始评估", use_container_width):
    ...
    if uploaded_files:
        ...
        if not all_dfs:
            ...
            try:
                ...
                st.dataframe(df_import.head(10), use_container_width)
        ...
        if view_mode == "Group by Patient ID":
            if "编号" not in df_display.columns:
                ...
                for patient_id, group_df in grouped_by_id:
                    ...
                    st.dataframe(group_df_show, use_container_width)
                    st.divider()
        else:
        ...
        if analysis_mode == "📊 Routine Indicator Analysis":
                ...
                if target_col is None and target_display != "Explanation":
                    ...
                    if target_display in categorical_fields:
                        ...
                        if len(series) == 0:
                            ...
                            st.pyplot(fig)
                            st.dataframe(cnt.rename("Count"), use_container_width)
                        ...
                        if len(s) == 0:
                            ...
                            st.dataframe(s.describe().round(2), use_container_width)
            ...
            if df_raw.empty:
                ...
                if not num_cols:
                        ...
                        try:
                            ...
                            if existing_cols:
                                ...
                                if len(X) > 5:
                                    ...
                                    st.dataframe(imp_df, use_container_width)
                    ...
                    st.dataframe(stats, use_container_width)
    st.stop()
        ...
        if len(X) < 10:
            ...
            if "outcome_model" in st.session_state:
                with st.expander("View Efficacy Model Feature Importance"):
                    ...
                    st.dataframe(imp_df_cls, use_container_width)
                    fig, ax = plt.subplots(figsize=(10, 6))
...
if st.button("✅ 生成个体化给药方案"):
    ...
    try:
        ...
        ml_adjusted = False           # 是否由ML处理了方案调整
                ...
                try:
                    ...
                    if pred_ml < target_low:      # 偏低，加强方案
                        ...
                        if best_safe:
                            ...
                            st.success(f"预测浓度偏高，已下调至 {剂量:.2f}g q{间隔}h 输注{输注_time}h，谷浓度≈{best_safe[6]:.2f} mg/L，%T>MIC {best_ft:.1f}%")
                        else:
                            st.warning("无法找到安全达标方案，维持原方案。")

                    else:   # 在理想范围，作为虚拟TDM进行贝叶斯反馈以统一模拟
                        ml_adjusted = True
                        # 将预测值当作虚拟TDM，进行贝叶斯估算但不改变剂量
                        ...
                        best_t_full, best_conc_full = t_full, conc_full
                        st.info("预测浓度在理想范围内，已匹配模拟曲线与预测值。")
                except Exception as e:
                    st.warning(f"ML预测失败（{e}），使用纯PPK方案。")
            else:
                st.warning("⚠️ 未训练PK机器学习模型，使用纯PPK方案。")

        # ---------- 如果未由ML处理，走原有贝叶斯+自动优化 ----------
        if not ml_adjusted:
        ...
        if crcl_val is not None:
            if crcl_val >= 130:
                ...
                explanation += f"🟡 中度肾功能不全（CrCl {crcl_val} mL/min），剂量由 {infection_dose}g 减量至 {renal_dose}g q{renal_interval}h。\n"
            elif 10 <= crcl_val < 26:
                explanation += f"🟠 重度肾功能不全（CrCl {crcl_val} mL/min），剂量减至 {renal_dose}g q{renal_interval}h。\n"
            else:
                explanation += f"🔴 终末期肾病（CrCl {crcl_val} mL/min），剂量减至 {renal_dose}g q{renal_interval}h。\n"
        else:
            missing = []
            if scr_val is None: missing.append("肌酐")
            if age_val is None: missing.append("年龄")
            if wt <= 0: missing.append("体重")
            if missing:
                explanation += f"⚠️ 缺少{', '.join(missing)}，按肾功能正常处理。\n"
            else:
                explanation += "⚠️ CrCl计算异常，按肾功能正常处理。\n"

        if ml_adjusted and 'pred_ml' in dir() and pred_ml is not None and c05h in ["", "无", None]:
            explanation += f"🤖 机器学习预测谷浓度：{pred_ml:.2f} mg/L。\n"
        if conclusion == "血药浓度过高":
            explanation += "⚠️ 实测浓度过高，存在神经毒性风险。"
        elif conclusion == "血药浓度过低":
            explanation += "⚠️ 当前方案浓度不足，可能影响疗效。"
        else:
            explanation += "✅ 当前方案达标，可维持。"
        ...
        if best_ft >= 90:
            ...
            explanation += "，未达90%。"
        if final_inf > 0.5:
            explanation += f" 输注时间延长至 {final_inf}h。"
        if final_interval != renal_interval:
            explanation += f" 间隔调整为 q{final_interval}h。"
        if final_dose != renal_dose:
            explanation += f" 剂量调整为 {final_dose}g。"
        ...
        record = {
            ...
            "目标MIC": target_mic, "达标概率": round(best_ft, 1),
            "建议": explanation,
            "record_id": str(uuid.uuid4())
        }
        st.session_state.patient_db.append(record)
        ...
        with colA:
            ...
            if crcl_val: st.metric("CrCl", f"{crcl_val} mL/min")
            if egfr_mdrd: st.metric("eGFR (MDRD)", f"{egfr_mdrd} mL/min/1.73m²")
        with colB:
            st.subheader("📉 血药浓度曲线（完整累积过程）")
            fig, ax = plt.subplots(figsize=(10, 5))
            ax.plot(t_full, conc_full, 'b-', lw=1.5, label='预测浓度')
            ax.axhline(target_mic, ls='--', color='red', lw=2, label=f'目标MIC={target_mic} mg/L')
            # 显示范围：0 到 10个给药间隔
            xmax = min(10 * final_interval, t_full[-1])
            ax.set_xlim(0, xmax)
            # 标注稳态峰谷（取最后3个周期）
            if np.any(mask_steady):
                conc_steady = conc_full[mask_steady]
                ...
                # 查找峰谷时间点
                peak_idx = np.argmax(conc_full)
                trough_idx = np.argmax(conc_full[-int(2 * cycle_len):]) + len(conc_full) - int(2 * cycle_len)
                ax.scatter(t_full[peak_idx], peak, color='green', s=80, zorder=5, label=f'峰浓度 {peak:.1f}')
                ax.scatter(t_full[trough_idx], trough, color='purple', s=60, zorder=5, label=f'谷浓度 {trough:.1f}')
            ax.set_xlabel('时间 (h)')
            ax.set_ylabel('血药浓度 (mg/L)')
            ax.set_title('美罗培南血药浓度累积曲线')
            ax.grid(True, linestyle=':', alpha=0.6)
            ax.legend(loc='upper right', fontsize=8)
            st.pyplot(fig)
        ...
        final_trough = pred_trough if pred_trough > 0 else conc_cycle[-1] if 'conc_cycle' in dir() else 0.0
        res = [
